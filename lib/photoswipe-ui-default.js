...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.ZVUIPunch_Default = factory();
  }
...
var _onControlsTap = function(e) {
    ...
    for (var i = 0; i < _uiElements.length; i++) {
      ...
      if (uiElement.onTap && clickedClass.indexOf('zvui-pinch_' + uiElement.name) > -1) {
        uiElement.onTap();
        found = true;
  ...
  _togglePswpClass = function(el, cName, add) {
    helper[(add ? 'add' : 'remove') + 'Class'](el, 'zvui-pinch_' + cName);
  },
  ...
  _toggleShareModal = function() {
    ...
    if (!_shareModalHidden) {
      ...
      setTimeout(function() {
        if (!_shareModalHidden) {
          helper.addClass(_shareModal, 'zvui-pinch_share-modal--fade-in');
        }
      }, 30);
    } else {
      helper.removeClass(_shareModal, 'zvui-pinch_share-modal--fade-in');
      setTimeout(function() {
        if (_shareModalHidden) {
          _toggleShareModalClass();
        }
  ...
  _openWindowPopup = function(e) {
    ...
    window.open(target.href, 'zvui-pinchshare', 'scrollbars=yes,resizable=yes,toolbar=no,' +
  ...
  _updateShareURLs = function() {
    ...
    for (var i = 0; i < _options.shareButtons.length; i++) {
      ...
      shareButtonOut += '<a href="' + shareURL + '" target="_blank" ' +
        'class="zvui-pinch_share--' + shareButtonData.id + '"' +
  ...
  _hasCloseClass = function(target) {
    for (var i = 0; i < _options.closeElClasses.length; i++) {
      if (helper.hasClass(target, 'zvui-pinch_' + _options.closeElClasses[i])) {
        return true;
      }
    }
  },
    ...
    if (_fitControlsInViewport()) {
      ...
      if (_options.captionEl && bars.bottom === 'auto') {
        if (!_fakeCaptionContainer) {
          _fakeCaptionContainer = helper.createEl('zvui-pinch_caption zvui-pinch_caption--fake');
          _fakeCaptionContainer.appendChild(helper.createEl('zvui-pinch_caption__center'));
          _controls.insertBefore(_fakeCaptionContainer, _captionContainer);
          helper.addClass(_controls, 'zvui-pinch_ui--fit');
        }
...
var _uiElements = [{
    ...
    option: 'arrowEl',
    onTap: zvuiPinch.prev
  }, {
    name: 'button--arrow--right',
    option: 'arrowEl',
    onTap: zvuiPinch.next
  }, {
    name: 'button--fs',
    option: 'fullscreenEl',
    onTap: function() {
...
var _setupUIElements = function() {
  ...
  var loopThroughChildElements = function(sChildren) {
    ...
    for (var i = 0; i < l; i++) {
      ...
      for (var a = 0; a < _uiElements.length; a++) {
        ...
        if (classAttr.indexOf('zvui-pinch_' + uiElement.name) > -1) {

          if (_options[uiElement.option]) { // if element is not disabled from options

            helper.removeClass(item, 'zvui-pinch_element--disabled');
            if (uiElement.onInit) {
            ...
            helper.addClass(item, 'zvui-pinch_element--disabled');
  ...
  var topBar = helper.getChildByClass(_controls, 'zvui-pinch_top-bar');
  if (topBar) {
...
ui.init = function() {
  ...
  // find zvui-pinch_ui element
  _controls = helper.getChildByClass(zvuiPinch.scrollWrap, 'zvui-pinch_ui');
  ...
  _listen('bindEvents', function() {
    helper.bind(_controls, 'zvuiTap click', _onControlsTap);
    helper.bind(zvuiPinch.scrollWrap, 'zvuiTap', ui.onGlobalTap);
  ...
  _listen('unbindEvents', function() {
    ...
    helper.unbind(_controls, 'zvuiTap click', _onControlsTap);
    helper.unbind(zvuiPinch.scrollWrap, 'zvuiTap', ui.onGlobalTap);
  ...
  _listen('destroy', function() {
    if (_options.captionEl) {
      ...
      helper.removeClass(_captionContainer, 'zvui-pinch_caption--empty');
    }

    if (_shareModal) {
      _shareModal.children[0].onclick = null;
    }
    helper.removeClass(_controls, 'zvui-pinch_ui--over-close');
    helper.addClass(_controls, 'zvui-pinch_ui--hidden');
    ui.setIdle(false);
  });


  if (!_options.showAnimationDuration) {
    helper.removeClass(_controls, 'zvui-pinch_ui--hidden');
  }
...
ui.update = function() {
  ...
  if (_controlsVisible && zvuiPinch.currItem) {
    ...
    if (_options.captionEl) {
      ...
      _togglePswpClass(_captionContainer, 'caption--empty', !zvuiPinch.currItem.title);
    }
...
ui.onGlobalTap = function(e) {
  ...
  if (e.detail && e.detail.pointerType === 'mouse') {
    ...
    if (helper.hasClass(target, 'zvui-pinch_img')) {
    ...
    if (_options.tapToClose && (helper.hasClass(target, 'zvui-pinch_img') || _hasCloseClass(target))) {
      zvuiPinch.close();
      return;
    }
